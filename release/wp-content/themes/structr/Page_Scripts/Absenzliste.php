     ...
     table2 = $('.datatables').DataTable( {
        ...
        ajax:     { 
            ...
            data: {
				...
				'sid': document . getElementById( "schueler" ) . value,
...

{
	...
	echo '<input  id="schueler" name="schueler" readonly="readonly" type="text" value="' . $Vorname . ' ' . $Name . ' ID:' . $value . '" />';
...
<br>
<select name="semester" id="semester" onchange="showTable()"  required="required">
   <?php
...
<br>
<br>
<h4>Gesamtüberblick der Abwesenheiten</h4>
<hr width=100%>
<div id="abwchart"></div>
...
<script>
	...
	function drawAbw(){
	  $.ajax({
        url:"/wp-content/themes/structr/Page_Scripts/getDataAbw2.php",
        method:"POST",
        data:{q: document . getElementById( "schueler" ) . value,
			 sem: document . getElementById( "semester" ) . value
			 },
        dataType:"JSON",
...
google.charts.setOnLoadCallback(drawChart4);
    ...
    data.addColumn('string', 'Kursname');
    data.addColumn('number', 'Abwesenheiten');
				
			
    $.each(jsonData, function(i, jsonData){
        var Kursname = jsonData.Kursname;
        var Abwesenheit = jsonData.Abwesenheit;
		
        data.addRows([[Kursname,Abwesenheit]]);
    });
  var options = {
          title : 'Abwesenheiten in den Kursen',
