...
$semester = $_GET['k'];
 ...
 $isEntryLP= "Select * From sv_Lehrpersonen where User_ID='$uid'";

    $resultLP = mysqli_query($con, $isEntryLP);

    while( $vLP= mysqli_fetch_array($resultLP)) {
        $lid=$vLP['ID'];
...
if ($semester==$semDB){
  ...
  $ZeitStdpln=$semester."_ZeitenStundenplan";
    ...
    $isEntry = "Select * From $KurseDB Where Lehrperson='$lid' and Tag='$Tag' ";
...
while( $value= mysqli_fetch_array($result))
{
    ...
    if ($dontwrite <>1 and substr($value['KursID'], -4)==$semester){
        ...
        $Farbe= $value['Farbe'];
        $Name=$y;
        echo '<input type="text" name='.$y.' style="width: 50px; height: 39px;background-color:'.$Farbe.'"; readonly >        <b>  KursID: </b> '.$value['KursID'].' |   <b>  Kursname: </b> '.$Kursname.'        |    <b>Lehrer:</b> '.$VornameLehrer.'  '.$NachnameLehrer.'   |    <b>Startdatum des Kurses:</b> '.$Startdatum.'<br/>';
    }
}
