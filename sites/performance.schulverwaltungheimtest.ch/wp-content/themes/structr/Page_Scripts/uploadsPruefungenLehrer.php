...
// CSV-Datei einlesen
   ...
   $isEntry1 = "SELECT * From sv_Lehrpersonen Where  User_ID='$User_ID' ";
...
while( $value1= mysqli_fetch_array($result1))
{
	$LP_ID=$value1['ID'];
	  ...
	  $isEntry2 = "SELECT * From sv_KurseLehrer Where  LP_ID='$LP_ID' ";
$result2 = mysqli_query($con, $isEntry2);

while( $value2= mysqli_fetch_array($result2))
{
	$KursID1=$value2['KursID'];
...
while( $value= mysqli_fetch_array($result))
{
	$Farbe=$value['Farbe'];
            ...
            if ($KursID == "" or $Klasse == "" or $Kursname == "" or $Pruefungsname == ""  or $Datum == "" or $Start == ""  or $Ende == "" or $Tag == ""  or $Gewichtung == ""   or $Lehrperson == "" or $LP_ID == "" ) {
                ...
                while ($value = mysqli_fetch_array($result)) {
                    ...
                    if (($value['Zimmer'] == $Zimmer) and ($value['Start'] == $Start) and ($value['Ende'] == $Ende) and ($value['KursID'] == $KursID) and ($value['Pruefungsname'] == $Pruefungsname)) {
                        ...
                        echo "Die Pruefung ".$Pruefungsname." existiert bereits im Kurs ".$KursID." und am Startdatum ".$Start."!";
...
//mysqli_query($con,$sql_befehl);
	...
	echo "Der Kurs ".$KursID." wurde nicht hochgeladen, da Sie als Lehrperson diesem Kurs nicht zugeordnet sind!";
}
...
//echo '<meta http-equiv="refresh" content="0; url=/pruefungen" />';
?>
  <button onclick="window.location.href='/pruefungen';">
      Zurück
    </button>
